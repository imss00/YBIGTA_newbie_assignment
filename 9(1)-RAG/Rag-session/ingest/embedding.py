...
def embed_passages(texts: list[str], ids: list[str], progress_callback=None) -> np.ndarray:
    ...
    -> 텍스트 리스트를 배치로 임베딩하여 저장하는 함수(AI가 이해하기 쉬운 형태로 벡터화하는 것)
    ...
    # 3. BATCH_SIZE(100개)씩 끊어서 처리 -> 1000번 호출 <<< 100번씩 10번 호출
    for i in range(0, total, BATCH_SIZE):
        batch_texts = texts[i : i + BATCH_SIZE]
        
        # 헬퍼 함수를 이용해 안전하게 임베딩 벡터를 받아옵니다
        batch_vectors = _embed_batch_safe(client, batch_texts)
        ...
        if progress_callback:
            ...
            print(f"Progress: {len(all_embeddings)}/{total}")

    # 3. 리스트를 numpy 배열로 변환 -> 벡터는 일반 텍스트보다 .npy같은 이진 파일로 불러오는 게 훨씬 빠름
...
def embed_query(query: str) -> list[float]:
    ...
    # 길면 잘라내기
    truncated_qeury = _truncate(query)

    # 쿼리 전용 모델 사용해서 임베딩 생성
    response = client.embeddings.create(
        model="solar-embedding-1-large-query",
        input=truncated_qeury
    )

    # 기본적으로 list[float] 형태이지만 
    embedding = response.data[0].embedding

    # 벡터만 추출해 변환해서 return
    return list(embedding) # 한 번 더 처리해주기
