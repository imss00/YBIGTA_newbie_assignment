...
def parse_status_and_preview(raw: bytes, max_preview: int = 200) -> tuple[Optional[int], str, Optional[str]]:
    ...
    ind = raw.find(b"\r\n\r\n")
    if ind == -1 : 
        return None, "", "Parsing Error: Header boundary not found"

    header_bytes = raw[:ind]
    body_bytes = raw[ind+4:]
    
    # 2. 상태 코드 추출
    try :
        # 1) decode 먼저 해서 문자열로 바꾸기
        ...
        if len(parts) < 2:
            return None, "", "Error: Invalid Status Line"
        
        status_code = int(parts[1])

    except ValueError:
        return None, "", "Error: Status code is not a number"


    # 3. preview 생성 (Decode를 해야 str의 형태가 됨)
