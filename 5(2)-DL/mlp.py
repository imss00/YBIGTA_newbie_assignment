...
class MultiLayerPerceptron(object):
    ...
    def forward_propagation(self, X):
        ...
        h3 = np.dot(z2, W3) + b3
        # 문제 명세: z3 = tanh(H3 + H1) -> Skip Connection 적용
        z3 = tanh(h3 + h1)
    ...
    def back_propagation(self, cache, X, y, L2_norm=0.0):
        ...
        # Layer 3: Tanh & Skip Connection handling
        # z3 = tanh(u), where u = h3 + h1
        # du = dz3 * (1 - z3^2)
        du = dz3 * (1 - np.power(z3, 2))
        
        # Gradient splits to both h3 and h1 (skip connection)
        dh3 = du
        dh1_skip = du 
        ...
        # Layer 2: LeakyReLU
        dz2 = dh3 @ W3.T
        # LeakyReLU derivative: 1 if x > 0 else 0.01
        dh2 = dz2.copy()
        dh2[h2 < 0] *= 0.01
        ...
        # Layer 1: ReLU & Incoming Skip Connection
        dz1 = dh2 @ W2.T
        # ReLU derivative
        dh1 = dz1.copy()
        dh1[h1 <= 0] = 0
        
        # *** 중요: Skip Connection으로 인한 Gradient 추가 ***
        dh1 += dh1_skip 
